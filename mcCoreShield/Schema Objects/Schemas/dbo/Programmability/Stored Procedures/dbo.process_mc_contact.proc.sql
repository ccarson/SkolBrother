...
************************************************************************************************************************************
  ...
  Procedure: dbo.process_mc_contact
     Author: Chris Carson
...
BEGIN
    ...
    --  2)  Assign existing Core.Contacts.id to matching incoming records

    --  3)  MERGE incoming data into Core.Contacts
         ...
         USING  #mc_contact   AS src
          ...
          WHEN  MATCHED THEN
                UPDATE
                   SET  salutation      = Salutation
                      ...
                      , tzOffset        = TzOffset
                      , iDefaultQuota   = iDefault_Quota
                      , iDocUsage       = iDoc_Usage
                      , assistID        = assist_id
                      , layout          = layout
                      , bTOS            = bTOS
          ...
          WHEN  NOT MATCHED BY TARGET THEN
                INSERT ( id, salutation, jobTitle, firstName, initial, lastName, suffix
                           , email, login, password, salt, accessID, expires, hits, lastLogin
                           , status, modifiedBy, datejoined, membertype, photo, resume, thumb
                           , PIN, reset, mailsent, sysmember, maildate, updatesent, updatenum
                           , nosend, hidden, securityLevel, review, q1, q2, q3, iAnswer, ipMac
                           , frequencyID, refer, isActive, timeZone, usesDaylight, tzOffset
                           , iDefaultQuota, iDocUsage, assistID, layout, bTOS, bOnlineNow, uID
                           , iwkgrplayout, sAboutMe, folderID, signature, bAuditLock, bProfileUpdate
                           , bexpirereminder, bPingSent, dPingDate, inetwork
