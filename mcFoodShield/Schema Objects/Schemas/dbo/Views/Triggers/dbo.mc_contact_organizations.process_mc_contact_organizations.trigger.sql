...
CREATE TRIGGER [dbo].[process_mc_contact_organizations] ON [dbo].[mc_contact_organizations]
...
BEGIN
	...
	SELECT  @tableID = id
	 ...
	 WHERE  tableName = 'mc_contact_organizations' ;
	...
	EXEC @rc = [mcCoreShield].[dbo].[mc_contact_organizations] 
		@systemID = @systemID
		, @tableID = @tableID
		, @recordsIN = @recordsIN
		, @dataXML = @dataXML
