...
CREATE TRIGGER [dbo].[process_mc_contact_verification] ON [dbo].[mc_contact_verification]
...
BEGIN
	...
	SELECT  @tableID = id
	 ...
	 WHERE  tableName = 'mc_contact_verification' ;
	...
	EXEC @rc = [mcCoreShield].[dbo].[mc_contact_verification] 
		@systemID = @systemID
		, @tableID = @tableID
		, @recordsIN = @recordsIN
		, @dataXML = @dataXML
